...
`palmID` is a contained analysis suite for viral RNA-dependent RNA
polymerases (RdRP) based on the "Palmprint" RNA virus barcodes described
by [Babaian and Edgar,
2021](https://www.biorxiv.org/content/10.1101/2021.03.02.433648v1).
...
Identification of the the core catalytic motifs A,B,C within the input
sequence and reporting the "palmprint" RNA-virus barcode.
...
Quality control metrics on the motif-scores and size distribution of the
"palmprint" informs the confidence that the input sequence contains a
viral RdRP. Input scores are shown compared to score distribution from
15,010 canonical viral RdRP from GenBank.
...
A `MUSCLE`-generated multiple sequence alignment of the top-10 matching
hits in [`palmdb`](https://github.com/rcedgar/palmdb).
...
Input "palmprint" are aligned against
[`palmdb`](https://github.com/rcedgar/palmdb) using `diamond` to
retrieve related viruses. Upto the top 500 related viruses are reported.
...
RNA viruses matching input are cross-referenced against 5.7M SRA
sequencing libraries to identify sra-virus matches and their associated
meta-data.

#### Geospatial distribution of related RNA viruses

Distribution of matching RNA viruses. Click to explore each matching
record. Species, genus, family and phylum level matches are red, orange,
...
Organism meta-data from the SRA sequencing libraries matching the
input-virus. Word size and color is scaled by proximity of the
input-virus to its match in the library. Wordclouds show a) Researcher
provided annotations of the libraries or b) STAT automated taxonomic
(orders) analysis of the reads contained within the library ([Katz et
al.,
2021](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-021-02490-0)).
