...
pro.df <- read.pro(input.pro)
  ...
  pro.df              <- get.proTax2(pro.df, con) # TODO OPTIMIZE proTax function
...
cbind( pro.df[, c("sseqid", "nickname", "pident", "evalue", "tspe", "gbid", "cigar")],
       blast.col) %>%
  DT::datatable(
    colnames = c('palm_id', 'nickname', 'input_id%','evalue', 'genbank_tax', 'genbank_id%', 'CIGAR', 'palm_BLAST'),
...
cbind( palm.sra[, c("run_id", "biosample_id", 'palm_id', 'pident', 'coverage', 'evalue', 'scientific_name')],
       blast.col) %>%
  DT::datatable(
    colnames = c("sra_run", "biosample_id", "palmprint", "palm_id%", "coverage", "evalue", "sra_scientific_name", "sra_seq_BLAST"),
